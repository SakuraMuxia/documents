...
for (Field field : clazz.getDeclaredFields()) {
    ...
    System.out.println("getType().getSimpleName()：" + field.getType().getSimpleName());
    System.out.println("------");
}
字段名：age
getType()：int
...
getType().getSimpleName()：Integer
------
包装类（例如 Integer, Double, Boolean, Character 等）在反射中是普通的 Java 类，都会显示全限定名。
```
