...
| 项目             | 数组（Array）           | List 集合                              |
| ---------------- | ----------------------- | -------------------------------------- |
| 所属包           | `java.lang`（内置类型） | `java.util`（集合框架）                |
| 存储类型         | 固定长度的顺序表        | 可变长度的容器                         |
| 元素类型         | 只能存储一种数据类型    | 可存储对象引用（泛型）                 |
| 长度             | 创建后**固定不变**      | 可**动态增删**元素                     |
| 下标访问         | 支持随机访问 `arr[i]`   | 支持 `list.get(i)`                     |
| 性能             | 速度快，占内存小        | 功能多，略慢一些                       |
| 是否支持泛型     | 不支持泛型              | 支持泛型（类型安全）                   |
| 常见实现类       | 无                      | `ArrayList`、`LinkedList`、`Vector` 等 |
| 是否支持自动扩容 | ❌ 不支持                | ✅ 自动扩容（例如 `ArrayList`）         |
